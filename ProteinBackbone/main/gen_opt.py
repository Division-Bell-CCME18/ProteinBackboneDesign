...
if __name__ == '__main__':
    ...
    data = transform(pdb_to_data(args.pdb_id + '.pdb'))
    perturb_data = gen_perturb(data, sigma_perturb=args.sigma_perturb, sigma_end=config.model.sigma_end)
    update_pdb_info(perturb_data, f'{args.pdb_id}_tidy.pdb', save_dir=args.working_dir ,suffix='perturb')
    print(f'save perturbed protein backbone structure done! saved as {args.pdb_id}_tidy_perturb.pdb')
    ...
    # write in pdb file
    update_pdb_info(return_data, f'{args.pdb_id}_tidy.pdb', save_dir=args.working_dir, suffix='opt')
    print(f'{args.pdb_id} backbone structure optimization done! save as {args.pdb_id}_tidy_opt.pdb')
