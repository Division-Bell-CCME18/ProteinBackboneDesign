...
if __name__ == '__main__':
    ...
    log_file = open('model_eval.log', mode='w', encoding='utf-8')
    sys.stdout = log_file
    ...
    def gen_pert_opt(pdb_id, working_dir, sigma_perturb, config, steps_pos=None):
        """
    ...
    if args.pdb_id != 'all':
        ...
        pdb_success = []
        try:
            perturb_rmsd, opt_rmsd = gen_pert_opt(pdb_id=args.pdb_id, working_dir=args.working_dir, sigma_perturb=args.sigma_perturb, config=config, steps_pos=args.steps_pos)
            if perturb_rmsd-opt_rmsd > 0:
                ...
                print('optimization of %s finished! (rmsd_change: %.3f < 0)\n' % (args.pdb_id, perturb_rmsd-opt_rmsd))
        except:
        ...
        for pdb_id in pdb_list:
            try:
                perturb_rmsd, opt_rmsd = gen_pert_opt(pdb_id=pdb_id, working_dir=args.working_dir, sigma_perturb=args.sigma_perturb, config=config, steps_pos=args.steps_pos)
                if perturb_rmsd-opt_rmsd > 0:
    ...
    print('optimization of %d structures succeeded!\n' % len(pdb_success))
